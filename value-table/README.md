...
| Lengths Value Table | Dynamic, spill-based; length zones + even/odd levers; hosts the trim helpers | Visible |
...
| Helpers | Thickness decimals, species names, width/length tokens | Visible |
...
3. Build the Helpers sheet (thickness -> decimal map, species list).
...
- Spill formulas need empty room below/right of the anchor, or you get #SPILL!.
  RESET clears spill zones before restoring.
- If you change a tab's formulas on purpose, update its backup or the next RESET
  reverts your change.
